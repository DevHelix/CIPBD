...
def parse_page(txt, page_num, cip_year):
    ...
    if len(lines) > 1:
        m = re.match(r'(.+?)\s+(\d{7,12})\s*$', lines[1].strip())
        if m:
    ...
    # Parse year columns from header line
    header_m = re.search(
        r'Funding Source\s+Beginning Balance\s+((?:FY \d{2}\s+)+)3 Year Total', txt
    )
...
def combine(cip_year):
    ...
    with pdfplumber.open(pdf_path) as pdf:
        for pg in pdf.pages:
            ...
            if 'Funding Source' not in txt or 'Beginning Balance' not in txt:
                continue
